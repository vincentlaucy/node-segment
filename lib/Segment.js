 ...
 * @return {Segment}
 */
var moduleCtx = require.context('./module',false,/.*$/);
var dicts = require.context('../dicts',false,/.*txt$/);
 ...
 * @return {Segment}
 */
 // TODO refactor isDataString flag
Segment.prototype.loadDict = function (dataSource, isDataString, type, convert_to_lower) {
  var data;
  console.log('load dict');
  //TODO external dict
  if (!isDataString){
    console.log(dataSource);
    data = dicts('./'+dataSource);
  } else {
    console.log('[source]');
    data = dataSource;
  }
 ...
 * @param {String} name 字典文件名
 */
Segment.prototype.loadStopwordDict = function (dataSource, isDataString) {
  var type = 'STOPWORD';
  ...
  // 导入数据
  // var data = fs.readFileSync(filename, 'utf8');
  var data = isDataString ? dataSource : dicts('./'+dataSource);
  data.split(/\r?\n/).forEach(function (line) {
    line = line.trim();
    if (line) {
      TABLE[line] = true;
